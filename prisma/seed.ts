...
async function main() {
  ...
  for (const category of categories) {
    const created = await prisma.category.upsert({
      where: { id: `cat-${category.name.toLowerCase().replace(/\s/g, '-')}` },
      update: {},
      create: {
  ...
  const settings = [
    ...
    { key: 'restaurant_phone', value: '021-12345678', category: 'general' },
    { key: 'restaurant_address', value: 'Jl. Raya Utama No. 100, Jakarta', category: 'general' },
